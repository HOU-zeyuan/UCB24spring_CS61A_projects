    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        for i in range(len(typed_words)):
            if typed_words[i] == source_words[i]:
                count += 1
        return round(count / len(typed_words) * 100.0, 1)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return round(len(typed) / 5 * elapsed / 60, 1)
    # END PROBLEM 4
...
def time_per_word(words, timestamps_per_player):
    ...
    Arguments:
        ...
        timestamps_per_player: A list of lists of timestamps including the time
            the player started typing, followed by the time the player finished
            typing each word.
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(len(timestamps_per_player[i]) - 1):
            time_i.append(timestamps_per_player[i][j + 1] - timestamps_per_player[i][j])
        times.append(time_i)
